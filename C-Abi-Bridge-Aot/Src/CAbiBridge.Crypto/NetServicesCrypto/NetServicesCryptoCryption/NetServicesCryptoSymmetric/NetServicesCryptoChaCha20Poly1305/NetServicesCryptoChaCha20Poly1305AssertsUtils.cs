...
public partial class NetServicesCrypto
{
    ...
    ReadOnlySpan<byte> bytes, UsIPtr<byte> key)
  {
    ...
    if (length > CHACHA_POLY_MAX_PLAIN_SIZE) throw new ArgumentOutOfRangeException(
      nameof(bytes),$"length max >> {nameof(CHACHA_POLY_MAX_PLAIN_SIZE)} = {CHACHA_POLY_MAX_PLAIN_SIZE}");
    if (length < CHACHA_POLY_MIN_PLAIN_SIZE) throw new ArgumentOutOfRangeException(
      nameof(bytes), $"length max >> {nameof(CHACHA_POLY_MIN_PLAIN_SIZE)} = {CHACHA_POLY_MIN_PLAIN_SIZE}");
