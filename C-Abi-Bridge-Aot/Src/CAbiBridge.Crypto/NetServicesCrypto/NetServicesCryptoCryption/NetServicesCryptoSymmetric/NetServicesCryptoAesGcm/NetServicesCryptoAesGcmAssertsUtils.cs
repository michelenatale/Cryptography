...
public partial class NetServicesCrypto
{
  ...
  ReadOnlySpan<byte> bytes, UsIPtr<byte> key)
  {
    ...
    if (length > AES_GCM_MAX_PLAIN_SIZE) throw new ArgumentOutOfRangeException(
      nameof(bytes), $"length max >> {nameof(AES_GCM_MAX_PLAIN_SIZE)} = {AES_GCM_MAX_PLAIN_SIZE}");
    if (length < AES_GCM_MIN_PLAIN_SIZE) throw new ArgumentOutOfRangeException(
      nameof(bytes), $"length min >> {nameof(AES_GCM_MIN_PLAIN_SIZE)} = {AES_GCM_MIN_PLAIN_SIZE}");
