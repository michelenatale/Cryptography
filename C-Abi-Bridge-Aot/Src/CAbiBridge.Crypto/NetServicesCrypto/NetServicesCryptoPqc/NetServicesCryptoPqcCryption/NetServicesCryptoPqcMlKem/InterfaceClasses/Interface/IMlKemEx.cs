...
public interface IMlKemEx
{
  ...
  /// </summary>
  /// <param name="kem">Desired MLKem-instance</param>
  /// <param name="capsulation">Out-Parameter, desired Capsulation (Ciphertext from the Encapsulation)</param>
  /// <returns>Returns the Shared Secred Key as bytes in a protected (fix) UsIPtr-instance</returns>
  static abstract UsIPtr<byte> ToSharedKey(MLKem kem, out byte[] capsulation);

  /// <summary>
  /// Creates a Shared Secred Key based on the entered parameters. (Decapsulation)
  /// </summary>
  /// <param name="kem">Desired MLKem-Instance</param>
  /// <param name="capsulation">Desired Capsulation (Ciphertext from the Encapsulation)</param>
  /// <returns>Returns the Shared Secred Key as bytes in a protected (fix) UsIPtr-instance</returns>
  static abstract UsIPtr<byte> ToSharedKey(MLKem kem, ReadOnlySpan<byte> capsulation);
