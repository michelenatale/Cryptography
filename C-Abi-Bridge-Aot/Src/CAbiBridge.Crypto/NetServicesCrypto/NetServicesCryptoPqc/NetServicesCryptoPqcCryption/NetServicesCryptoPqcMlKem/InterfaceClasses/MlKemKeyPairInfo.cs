...
using Pointers;
using static NetServicesCrypto.JSSerializer;
...
public sealed class MlKemKeyPairInfo : IMlKemKeyPairInfo
{
  ...
  public MLKemAlgorithm ToAlgo() =>
    ToMLKemAlgorithm(this.Parameter);
  ...
  private void SetParameters(MlKemKeyPairInfo info) =>
    this.SetParameters(info.Id, info.PubKey,
      info.PrivKey, ToMLKemAlgorithm(info.Parameter), info.CryptAlgo);
    ...
    CryptionAlgorithm cryptoalgo)
  {
    ...
    this.Parameter = FromMLKemAlgorithm(parameter);
    if (id == Guid.Empty) return;
    this.Id = id;
  }

  public static MLKemAlgorithm ToMLKemAlgorithm(MLKemParam param) =>
    NetServicesCrypto.ToMLKemAlgorithm(param);

  public static MLKemParam FromMLKemAlgorithm(MLKemAlgorithm parameter) =>
    NetServicesCrypto.FromMLKemAlgorithm(parameter);
