...
public sealed class MlDsaKeyPairInfo : IMlDsaKeyPairInfo
{
  ...
  public void SaveKeyPair(string filename, bool with_privkey)
  {
    ...
    using var info = new MlDsaKeyPairInfo(this); //copy
    if (!with_privkey) info.PrivKey = [];

    var b64 = Convert.ToBase64String(
      SerializeJsonMlDsa(info));
  ...
  {
    this.Clear();
    using var info = Load_KeyPair(filename);
    this.SetParameters(info);
  }
  ...
  private void SetParameters(MlDsaKeyPairInfo info) =>
    this.SetParameters(info.Id, info.PubKey,
      info.PrivKey, ToMLDsaAlgorithm(info.Parameter));
    ...
    MLDsaAlgorithm parameter)
  {
    ...
    this.Parameter = FromMLDsaAlgorithm(parameter);
  ...
  public static MLDsaAlgorithm ToMLDsaAlgorithm(
    MLDsaParam param) =>
      NetServicesCrypto.ToMLDsaAlgorithm(param);

  public static MLDsaParam FromMLDsaAlgorithm(
    MLDsaAlgorithm parameter) =>
      NetServicesCrypto.FromMLDsaAlgorithm(parameter);
  ...
  ~MlDsaKeyPairInfo() => this.Dispose(false);
