...
partial class CryptoChaCha20Poly1305Test
{
  ...
  public static void TestChaCha20Poly1305BytesStress()
  {
    ...
    var tag = new byte[NetServicesCrypto.CHACHA_POLY_TAG_SIZE];
    var nonce = RngBytes(NetServicesCrypto.CHACHA_POLY_NONCE_SIZE);
    var plain = RngBytes(NetServicesCrypto.CHACHA_POLY_MAX_PLAIN_SIZE);
    var associat = Encoding.UTF8.GetBytes("© Michele Natale 2021");
  ...
  {
    Console.Write($"{nameof(TestChaCha20Poly1305BytesManaged)}Aot: ");
  ...
  {
    Console.Write($"{nameof(TestChaCha20Poly1305BytesStressManaged)}Aot: ");
