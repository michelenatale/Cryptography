...
partial class CryptoAesTest
{
  ...
  {
    Console.Write($"{nameof(TestAesFile)}Aot: ");
    ...
    for (var i = 0; i < rounds; i++)
    {
      ...
      using var key = new UsIPtr<byte>(RngBytes(NetServicesCrypto.AES_KEY_SIZE));
  ...
  {
    Console.Write($"{nameof(TestAesFileManaged)}Aot: ");
    ...
    for (var i = 0; i < rounds; i++)
    {
      ...
      using var key = new UsIPtr<byte>(RngBytes(
        NetServicesCrypto.AES_KEY_SIZE));

      var err = CryptoBridge.AesEncryptFileManaged(
        src, dest, key, associated);
      AssertError(err);

      err = CryptoBridge.AesDecryptFileManaged(
