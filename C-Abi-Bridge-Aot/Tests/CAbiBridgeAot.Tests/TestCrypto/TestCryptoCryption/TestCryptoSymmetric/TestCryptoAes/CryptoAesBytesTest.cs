...
partial class CryptoAesTest
{
  ...
  public static void TestAesBytes(int rounds)
  {
    ...
    for (int i = 0; i < rounds; i++)
    {
      ...
      var tag = new byte[NetServicesCrypto.AES_TAG_SIZE];
      var nonce = RngBytes(NetServicesCrypto.AES_IV_SIZE);

      size = rand.Next(
        NetServicesCrypto.AES_MIN_PLAIN_SIZE,
  ...
  {
    Console.Write($"{nameof(TestAesBytesManaged)}Aot: ");
  ...
  {
    Console.Write($"{nameof(TestAesBytesStressManaged)}Aot: ");
